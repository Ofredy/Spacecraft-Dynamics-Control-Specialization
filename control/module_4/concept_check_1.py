...
def get_att_track_control_k(x_k, t):
    ...
    control_torque = (
        ...
        + helper_functions.get_tilde_matrix(w_b_n_k) @ inertia_tensor @ w_b_n_k
    )

    for idx, control in enumerate(control_torque):

        if abs(control) >= 1.0:

            control_torque[idx] = 1.0 * np.sign(control)
...
def get_att_track_state_dot(x_k, t):
    ...
    w_b_n_dot = np.linalg.inv(inertia_tensor) @ ( get_att_track_control_k(x_k, t) + unmodeled_torque - helper_functions.get_tilde_matrix(w_b_n_k) @ inertia_tensor @ w_b_n_k ) 
...
get_track_error_at_time(mrp_sum, 60, dt=dt)